...
//CA Paging VMI tool to monitor gva-to-hpa mapping contiguity of guest tasks 
//through a procfs interface (/proc/VMI/<qemu_pid>/map,pagemap --> fs/proc/VMI.c)

// VMI tool help routines to get gva-to-hpa mappings
// guest page tables walk to get gva-to-gpa translation
int VMI_guest_page_table_walk(struct kvm_vcpu *vcpu, gva_t gva, u64 cr3, gfn_t *gfn, pteval_t *flags, int *level){
  gpa_t gpa;
  
  gpa = paging64_VMI_gva_to_gpa(vcpu, gva, PFERR_USER_MASK, NULL, cr3, flags, level);
...
EXPORT_SYMBOL(VMI_guest_page_table_walk);

// shadow page tables walk to get gpa-to-hpa translation
int VMI_shadow_page_table_walk(struct kvm_vcpu *vcpu, gpa_t gpa, hfn_t *pfn, pteval_t *flags, int *spte_level){
  ...
  level = mapping_level(vcpu, gpa >> PAGE_SHIFT, &force_pt_level);
  walk_shadow_page_lockless_begin(vcpu);
  ...
  if(is_large_pte(spte)) *spte_level=0x2;
  else *spte_level=0x1;

  *flags = spte & (shadow_user_mask|shadow_x_mask|shadow_nx_mask|PT_WRITABLE_MASK|SPTE_MMU_WRITEABLE|SPTE_HOST_WRITEABLE);
  
  if(is_large_pte(spte)) {
    *pfn = (spte_to_pfn(spte) + ((gpa>>PAGE_SHIFT)&(KVM_PAGES_PER_HPAGE(*spte_level) - 1)));
  }
  else *pfn = spte_to_pfn(spte);
...
EXPORT_SYMBOL(VMI_shadow_page_table_walk);
